...
/** A ColorFlowVectorPanel accelerated using OpenGL. "ColorFlow" here is a
  ...
  */
class VectorColorFlowGL(target: AnyRef, fShape: Shape, tShape: Shape)
        extends SGLCanvas
...
//        with Zoomable
{
  ...
  require(fShape.dimensions == 2, "[ColorFlowGL] Only 2D VectorFields are currently supported")
  require(tShape == Shape(2), "[ClorFlowGL] Only 2D vectors are supported.")
  ...
  // to round then, since we may as well make our array as small as possible.
...
//    pmvMatrix.destroy()
  ...
  var zDelta = 1f
  def changeZoomLevel(delta: Float) { Console.err.println("Zoom not yet implemented for OpenGL accelerated ColorFlow.")}

  /**
   * Current zoom level. Many panels display data in a grid of tiles or cells;
   * in those cases, zoomLevel is how many pixels tall and wide each cell is.
   * The presence of this field allows us to save and restore zoom levels
   * across program invocations.
   */
  def zoomLevel: Int = 1

  def zoomLevel_=(zoom: Int) {}
}

object VectorColorFlowGL {
...
class ColorFlowGLSubpanel(fShape:Shape, tShape: Shape)
        extends BorderPanel
        //with EventDrivenViewer
//        with Zoomable
{
  ...
  val colorFlow = new VectorColorFlowGL(this, fShape, tShape)
  val key = {
...
object ColorFlowGLSubpanel {
  val keyField: VectorFieldMemory = {
    val (keyRows, keyCols) = (64 | 1, 64 | 1)
    val (cRow, cCol) = (keyRows / 2, keyCols / 2)
    val maxLen = cRow min cCol
    val normalize = 1f / maxLen
    val keyFieldType = new FieldType(Shape(keyRows, keyCols), Shape(2), Float32)
    val mem = FieldMemory.direct(keyFieldType).asInstanceOf[VectorFieldMemory]
