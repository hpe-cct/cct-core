...
import cogx.compiler.parser.op.{PipelinedSensorOp, SensorOp}
import cogx.platform.cpumemory.ScalarFieldMemory
import cogx.runtime.ComputeGraphRestorerState

/** Kernel that copies user-supplied data to a buffer for use by consumers on the GPU.
  *
  ...
  */
private[cogx]
class SensorKernel(fieldType: FieldType, op: SensorOp, sensorClassname: String, restoreParameters: () => String)
  ...
  with Saveable
{
  ...
  private def compute(in: Array[OpenCLFieldRegister], out: OpenCLFieldRegister, fromReset: Boolean) {
    ...
    nextInput() match {
      case Some(writeData) =>
        ...
        val cpuMemory = out.master.cpuMemory.asInstanceOf[ScalarFieldMemory]
        if (writeData.isInstanceOf[Iterator[_]])
          cpuMemory.write(writeData.asInstanceOf[Iterator[Float]])
        else
          cpuMemory.write(writeData.asInstanceOf[Array[_]])
  ...
  def copyWithNewInputs(inputs: Array[VirtualFieldRegister]): AbstractKernel =
    new SensorKernel(fieldType, op, sensorClassname, restoreParameters)
