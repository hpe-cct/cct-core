...
/**
  ...
  */
private[cogx]
class UnpipelinedActuatorKernel(in: VirtualFieldRegister, op: UnpipelinedActuatorOp,
        ...
        with Saveable
{
  ...
  def compute(in: Array[OpenCLFieldRegister], out: Array[OpenCLFieldRegister]) {
    ...
    val cpuMemory = in(0).slave.read.asInstanceOf[ScalarFieldMemory]

    // Pass the data to the user-defined `nextOutput` function by providing the ScalarFieldReader object
    op.newOutput(cpuMemory)
  }
  ...
  def copyWithNewInputs(inputs: Array[VirtualFieldRegister]): AbstractKernel = {
    ...
    new UnpipelinedActuatorKernel(in, op, actuatorClassname, restoreParameters)
  }
