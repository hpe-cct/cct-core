...
import cogx.compiler.parser.op.ConvolveRows2DOp


/** Convolve the rows of a field with a filter.  Used in conjunction with
  * ConvolveColumns2DHyperKernel to implement separable convolution.
  *
  ...
  */
private[cogx]
class ConvolveRows2DHyperKernel private (in: VirtualFieldRegister,
                                         filter: VirtualFieldRegister,
                                         operation: ConvolveRows2DOp,
  ...
  code append """
    ...
    for (int i = 0; i < %filterSize%; i++) {
      ...
      if (%convolution%)
         ...
         column = i;
      sum += localImage[_localRow][c++] * readNonlocal(@in1);
    }
...
private[cogx]
object ConvolveRows2DHyperKernel {
  /** Convolve the rows of a field with a filter.
    ...
    */
  def apply(in: VirtualFieldRegister, filter: VirtualFieldRegister, operation: ConvolveRows2DOp,
            resultType: FieldType): HyperKernel =
  {
    ...
    operation.borderPolicy match {
      ...
      case BorderCyclic =>
      case x =>
        require(false, "unsupported border policy: " + x)
    }
    ...
    new ConvolveRows2DHyperKernel(in, filter, operation, resultType)
  }
}
