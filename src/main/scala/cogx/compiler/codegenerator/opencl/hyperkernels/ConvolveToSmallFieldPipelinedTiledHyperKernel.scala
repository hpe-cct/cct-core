...
/** Convolve a field with a dynamic filter field.
  ...
  */
private[cogx]
class ConvolveToSmallFieldPipelinedTiledHyperKernel private (inputs: Array[VirtualFieldRegister],
                                          operation: ConvolveToSmallFieldTiledOp,
    ...
    with Logarithm
{
  ...
  // A side-experiment confirms that cacheing the first input is not worth the extra code
  ...
  // The number of consecutive rows a single thread processes in its "minitile" of the "filter" (the reconstructed input).
  val miniTileRows = operation.rowsPerThread
  val workFieldColumns = filterColumns

  // We will adapt the kernel to handle non-mulitple filterRows
  // Throw exception for now
//  require(filterRows % miniTileRows == 0, "Kernel not ready to handle this case")
  val workFieldRows = (filterRows + miniTileRows - 1)/miniTileRows

  override lazy val workFieldType = filterType.
    resize(Shape(workFieldRows, workFieldColumns)).
    resizeTensor(Shape(planesPerImage * numLogicalFilters * batchSize))
  ...
  // Pipelined reduction occurs in a memory twice the size as the number of
  ...
  code append s"        int logicalOutputPlane = _tensorElement;\n"
  ...
  code append s"        int groupStartRow = get_group_id(1) * ${localRows * miniTileRows * upsampleStep};\n"
  ...
  /** Read in the filter, each thread gets its own values (miniTileRows in number) */
  code append (0 until miniTileRows).toArray.map(i => s"filterVal_$i = 0.0f").mkString("    float " , ", " , ";\n")

  def filterReadIn(flip: Boolean) {
    // The miniTileRows may not divide the filterRows evenly.
    // Detect this case and prevent out-of-bounds reading of the filter input.
    val lastValidModulus = (filterRows - 1) % miniTileRows
    var prunedModuli = false
    var indent = ""

    code append s"    if (_row * $miniTileRows < $filterRows && _column < $filterColumns) {\n"
    if (flip)
      code append s"        column = ${filterColumns - 1} - _column;\n"
    else
      code append "        column = _column;\n"
    for (i <- 0 until miniTileRows) {
      if (i == lastValidModulus + 1) {
        code append s"        if (_row * $miniTileRows + ${miniTileRows - 1} < $filterRows) {\n"
        prunedModuli = true
        indent = "    "
      }
      if (flip)
        code append indent + s"        row = ${filterRows - 1} - _row * $miniTileRows - $i;\n"
      else
        code append indent + s"        row = _row * $miniTileRows + $i;\n"
      code append indent + s"        filterVal_$i = readNonlocal(@in1);\n"
    }
    if (prunedModuli)
      code append "        }\n"
    code append "    }\n"
  ...
  code append "    // Filter read-in to thread-private filterVal variables\n"
  filterOrientation match {
    case CrossCorrelationOrientation => filterReadIn(flip = false)
    case ConvolutionOrientation => filterReadIn(flip = true)
  }
...
//  code append "    row = 0;\n"
  ...
  code append (0 until miniTileRows).toArray.map(i => s"inputVal_$i = 0.0f").mkString("    float " , ", " , ";\n")
  def iterationCode(i: Int, indentSize: Int = 4) {
    ...
    if (i < resultPoints ) {
      // Consecutive iterations walk down a column to leverage already read input values (assuming upsampleStep == 1)
      val inColumn = i / resultRows
      val inRow = i - inColumn * resultRows
      if (upsampleStep == 1) {
        if (inRow == 0)
          for (tileIndex <- 0 until miniTileRows)
            code append indent + s"inputVal_$tileIndex = localTile[baseInputIndex+$tileIndex*${paddedLocalColumns * upsampleStep}];\n"
        else {
          val lastRowIndex = inRow + miniTileRows - 1
          code append "// Problem line-\n"
          code append indent + s"inputVal_${lastRowIndex % miniTileRows} = localTile[baseInputIndex+$lastRowIndex*${paddedLocalColumns * upsampleStep}];\n"
        }

        code append indent + (0 until miniTileRows).map(tileIndex =>
          s"inputVal_${(inRow + tileIndex) % miniTileRows}*filterVal_$tileIndex").mkString(
          readMem + s"[$localWorkSize+threadIndex] = ",
          " + ",
          ";\n"
        )
      }
      else {
        // No practical way to leverage previous read inputs, since the tile shifts by 1 and the filterVals are separated by upsampleStep
        // Read in `miniTileRows` new inputs on each iteration.
        for (tileIndex <- 0 until miniTileRows)
          code append indent + s"inputVal_$tileIndex = localTile[baseInputIndex + ${inRow + tileIndex * upsampleStep} * ${paddedLocalColumns}];\n"
        code append indent + (0 until miniTileRows).map(tileIndex =>
          s"inputVal_$tileIndex*filterVal_$tileIndex").mkString(
          readMem + s"[$localWorkSize+threadIndex] = ",
          " + ",
          ";\n"
        )
      }
      if (inRow == resultRows - 1)
        code append indent + s"baseInputIndex += $downsampleStep;\n"
    }
  ...
  code append s"    int baseInputIndex = firstProductiveElementIndex + _localRow * ${paddedLocalColumns * miniTileRows * upsampleStep} + _localColumn * $upsampleStep;\n"
  ...
  // Since consecutive rows will have a different "buffer polarity" if there are
  // an odd number of columns per row, we handle rows in pairs.

  def loopedIterationCode(startIteration: Int, untilIteration: Int) {
    require (startIteration % resultRows == 0)
    require (untilIteration % resultRows == 0)
    val startColumn = startIteration / resultRows
    val untilColumn = untilIteration / resultRows
    code append "\n    // partially unrolled loop iterations " + startIteration +
            " through " + (untilIteration - 1) + "\n\n"
    code append s"    for (int inColumn = $startColumn; inColumn < $untilColumn; inColumn++) {\n"
    for (i <- startIteration until startIteration + resultRows)
      iterationCode(i, 8)
    if (resultRows % 2 == 1 && startIteration + resultRows < untilIteration) {
      code append "\n        inColumn++;\n"
      code append s"        if (inColumn < $untilColumn) {\n"
      for (i <- startIteration + resultRows until startIteration + 2 * resultRows)
        iterationCode(i, 12)
      code append "        }\n"
    }
    code append "    }\n"
...
//  val MaxUnrolledLoopIterations = 1
  val MaxUnrolledLoopIterations = 100
//  val MaxUnrolledLoopIterations = 50

  val firstNormalColumnStart = HyperKernel.roundUp(resultRows, pipeDepth).toInt
  if (loopIterations <= MaxUnrolledLoopIterations ||
    firstNormalColumnStart >= resultPoints) {
    // All iterations unrolled
    for (i <- 0 until loopIterations)
      iterationCode(i)
  } else {
    for (i <- 0 until firstNormalColumnStart)
      iterationCode(i)
    // A central core of iterations expressed as a loop
    loopedIterationCode(firstNormalColumnStart, resultPoints)
  ...
  code append s"        column = outIndex / $resultRows;\n"
  code append s"        row = outIndex - column * $resultRows;\n"
  code append s"        @outElementNonlocal0 = results[outIndex];\n"
  code append s"    }"

  /** Make textual substitutions for various parameters */
...
/** Factory object for creating kernels of this type.
  */
private[cogx]
object ConvolveToSmallFieldPipelinedTiledHyperKernel extends Logarithm {

  val DefaultLocalRows = 8
  val DefaultLocalColumns = 32
  val DefaultRowsPerThread = 2
  ...
  def apply(inputs: Array[VirtualFieldRegister], operation: AbstractConvolveOp,
    ...
    val convolveOp = operation match {
      ...
      case op: ConvolveTiledOp =>
        ConvolveOp(op.borderPolicy, op.filterOrientation, op.samplingPolicy, op.vectorMode, op.batchSize)
      case op: ConvolveToSmallFieldTiledOp =>
        ConvolveOp(op.borderPolicy, op.filterOrientation, op.samplingPolicy, op.vectorMode, op.batchSize)
    }

    val filterColumns = inputs(1).fieldType.columns
    val filterRows = inputs(1).fieldType.rows

    val bestLocalColumns = if (filterColumns <= 16) 16 else DefaultLocalColumns
    val bestLocalRows =
      math.min(roundUpPowerOf2(math.ceil(filterRows.toDouble/DefaultRowsPerThread).toFloat), DefaultLocalRows)
    val toSmallFieldConvolveOp  = operation match {
      case op: ConvolveOp =>
        ConvolveToSmallFieldTiledOp(bestLocalRows, bestLocalColumns, DefaultRowsPerThread,
          op.borderPolicy, op.filterOrientation, op.samplingPolicy, op.vectorMode, op.batchSize)
      case op: ConvolveTiledOp =>
        ConvolveToSmallFieldTiledOp(bestLocalRows, bestLocalColumns, DefaultRowsPerThread,
          op.borderPolicy, op.filterOrientation, op.samplingPolicy, op.vectorMode, op.batchSize)
      case op: ConvolveToSmallFieldTiledOp =>  op
    }
    ...
    val params =
      new ConvolutionToSmallFieldParams(inputs, convolveOp, resultType,
        platformParams,
        filterVolumeMultiplier = 2,
        numFilterMemories = 2,
        numResultMemories = 1,
        miniTileRows = toSmallFieldConvolveOp.rowsPerThread,
        _localColumns = toSmallFieldConvolveOp.localColumns,
        _localRows = toSmallFieldConvolveOp.localRows)
    import params._
    if (numFilterTiles == 1)
      new ConvolveToSmallFieldPipelinedTiledHyperKernel(inputs, toSmallFieldConvolveOp, resultType, TensorElementAddressing, params)
    else {
      ...
      val convolveKernel =
        new ConvolveToSmallFieldPipelinedTiledHyperKernel(inputs, toSmallFieldConvolveOp,
  ...
  def outFieldType(in0Type: FieldType, filterType: FieldType) =
    in0Type.resize(Shape(in0Type.fieldShape(0) - filterType.fieldShape(0) + 1,
      ...
      resizeTensor(Shape(in0Type.tensorShape.points * filterType.tensorShape.points))

  def isRecommended(inputs: Array[VirtualFieldRegister]): Boolean = {
    val filterPoints = inputs(1).fieldType.fieldShape.points
    val filterTensorPoints = inputs(1).fieldType.tensorShape.points
    val inputTensorPoints = inputs(0).fieldType.tensorShape.points
    val workGroups = math.ceil(filterPoints.toFloat / (DefaultLocalRows * DefaultLocalColumns * DefaultRowsPerThread))

    val workGroupsForGoodGpuUtilization = 30
    val retVal = filterTensorPoints * inputTensorPoints * workGroups >= workGroupsForGoodGpuUtilization
    retVal
  }
}
