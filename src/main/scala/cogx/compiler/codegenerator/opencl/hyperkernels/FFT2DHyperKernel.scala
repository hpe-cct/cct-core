...
import cogx.compiler.codegenerator.opencl.hyperkernels.fastfouriertransform.{Forward, WorkDimensions, FFT2DKernelCache}
import cogx.platform.types.{Opcode, VirtualFieldRegister, FieldType}
import cogx.cogmath.geometry.Shape
import cogx.compiler.parser.op.{FFT2DSubOpRI, FFTOpRI, FFTOp, FFT2DSubOp}
import cogx.compiler.codegenerator.common.FieldPolicies._

/** Performs one pass of a 2D FFT.
  *
  ...
  * @param in The virtual field registers of the input field to be 2D FFT'd.
  ...
  */
private[cogx]
class FFT2DHyperKernel private[hyperkernels] (in: Array[VirtualFieldRegister],
                                              opcode: Opcode,
  ...
  override lazy val workFieldType = new FieldType(workFieldShape, Shape(), resultTypes(0).elementType)
  ...
  val (splitRealImaginary, dir, scaleFactor) = opcode match {
    case x: FFT2DSubOp => (false, x.dir, x.scaleFactor)
    case y: FFT2DSubOpRI => (true, y.dir, y.scaleFactor)
...
/** Factory object for creating a chain of kernels to perform a 2D FFT
  */
private[cogx]
object FFT2DHyperKernel {
  val DeviceMaxWorkItemsPerWorkGroup = 256

  /** Create kernel DAG for 2D FFT.
    *
    * @param in The virtual field registers of the input field to be 2D FFT'd.
    ...
    HyperKernel =
  {
    ...
    val fftRows = inType0.fieldShape(0)
    val fftColumns = inType0.fieldShape(1)
    val workDimensions = FFT2DKernelCache.workDimensions(fftRows, fftColumns,
      DeviceMaxWorkItemsPerWorkGroup)
    val sourceCodes = FFT2DKernelCache.sourceCodes(fftRows, fftColumns,
      DeviceMaxWorkItemsPerWorkGroup)
    val kernelNames = FFT2DKernelCache.kernelNames(fftRows, fftColumns,
      DeviceMaxWorkItemsPerWorkGroup)
    ...
    def kernelChainUpToPass(pass: Int): HyperKernel = {
      ...
      if (splitRealImaginary)
        new FFT2DHyperKernel(input, FFT2DSubOpRI(pass, dir, scale), resultFieldTypes,
          BigTensorAddressing, workDimensions(pass), sourceCodes(pass))
      else
        new FFT2DHyperKernel(input, FFT2DSubOp(pass, dir, scale), resultFieldTypes,
          BigTensorAddressing, workDimensions(pass), sourceCodes(pass))
    }
