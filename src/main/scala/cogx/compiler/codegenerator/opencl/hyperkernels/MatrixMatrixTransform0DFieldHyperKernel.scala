...
/** Computes the matrix multiply of 2 fields considered to be matrices.
  ...
  */
private[cogx]
class MatrixMatrixTransform0DFieldHyperKernel(in: Array[VirtualFieldRegister],
        ...
        extends HyperKernel(op, in, resultType, SmallTensorAddressing)
{
  ...
  while (workGroupRows/2 >= resultRows) {
    workGroupRows /= 2
    if (workGroupColumns < resultColumns)
      workGroupColumns *= 2
  }
  while (workGroupColumns/2 >= resultColumns) {
    workGroupColumns /= 2
    if (workGroupRows < resultRows)
      workGroupRows *= 2
  }

  override lazy val workFieldType = new FieldType(resultShape, Shape(), resultType.elementType)
  ...
  val tile1Reads = Math.ceil(dotProductSize.toDouble/workGroupColumns).toInt
  val tile2Reads = Math.ceil(dotProductSize.toDouble/workGroupRows).toInt

  val minTileReads = Math.min(tile1Reads, tile2Reads)

  val innerLoopReads = Math.max(workGroupColumns/workGroupRows, workGroupRows/workGroupColumns)

  val innerLoopFMAs = Math.min(workGroupRows, workGroupColumns)
  ...
  val bankConflictKiller =
    if (workGroupColumns > 1 && workGroupRows > 1) {
  ...
  codeAppend(s"__local float tile1[$workGroupRows][$workGroupColumns$bankConflictKiller];\n")
  codeAppend(s"__local float tile2[$workGroupRows][$workGroupColumns$bankConflictKiller];\n")
  ...
  codeAppend(s"float retVal = 0.0f;\n")
  // innerLoopSum holds the dot-product delta accumulated by one tile
  codeAppend(s"float innerLoopSum = 0.0f;\n\n")
  ...
  def readInputTile(inputNum: Int, tileOriginRow: String, tileOriginColumn: String): Unit = {
    ...
    if (transpose)
      codeAppend(s"    tile$inputNum[transposedLocalColumn][transposedLocalRow] = \n")
    else
      codeAppend(s"    tile$inputNum[_localRow][_localColumn] = \n")
  ...
  if (minTileReads == tile1Reads)
    readInputTile(1, "_row - _localRow", "i * _localColumns")
  else
    readInputTile(2, "i * _localRows", "_column - _localColumn")
  ...
  codeAppend(s"\n")
  if (minTileReads == tile1Reads)
    readInputTile(2, s"(i * $innerLoopReads + j) * _localRows", "_column - _localColumn")
  else
    readInputTile(1, "_row - _localRow", s"(i * $innerLoopReads + j) * _localColumns")
  ...
  codeAppend(s"if (_row < _rows && _column < _columns) {\n")
  codeAppend(s"    tensorElement = _row * $resultColumns + _column;\n")
  codeAppend(s"    @outElementNonlocal0 = retVal;\n")
  codeAppend(s"}\n")
