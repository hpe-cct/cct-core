...
import cogx.platform.cpumemory.{ComplexVectorFieldMemory, FieldMemory}
import cogx.platform.types.FieldType
import cogx.platform.types.ElementTypes.Float32
import cogx.cogmath.geometry.Shape
import cogx.cogmath.algebra.complex.ComplexVector
import cogx.platform.cpumemory.readerwriter.ComplexVectorFieldWriter
...
/** A proxy for a VectorFieldWriter that lets the actual writer be changed at
  ...
  */
private[cogx]
class ComplexVectorFieldWriterProxy
        extends FieldWriterProxy
        with ComplexVectorFieldWriter
{
  /** The writer to which all write commands are directed. */
  private var actualWriter: ComplexVectorFieldWriter = null

  /** Change the proxied ScalarFieldWriter to `writer`. */
  private[cpu_operator] def setWriter(writer: ComplexVectorFieldWriter) {
    actualWriter = writer
  }
  ...
  override def setShape(fieldShape: Shape, vectorShape: Shape) {
    if (actualWriter == null) {
      ...
      actualWriter = FieldMemory.indirect(_fieldType).asInstanceOf[ComplexVectorFieldMemory]
    }
  }

  /** Write `value` to a 0D scalar field. */
  def write(value: ComplexVector) {
    actualWriter.write(value)
  }

  /** Write `value` at (`col`) in a 1D scalar field. */
  def write(col: Int, value: ComplexVector) {
    actualWriter.write(col, value)
  }

  /** Write `value` at (`row`, `col`) in a 2D scalar field. */
  def write(row: Int, col: Int, value: ComplexVector) {
    actualWriter.write(row, col, value)
  }

  /** Write `value` at (`layer`, `row`, `col`) in a 3D scalar field. */
  def write(layer: Int, row: Int, col: Int, value: ComplexVector) {
