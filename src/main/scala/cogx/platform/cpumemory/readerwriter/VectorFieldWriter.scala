...
/** The interface for writing a vector field on the CPU.
  ...
  * @author Greg Snider
  */
trait VectorFieldWriter extends FieldWriter
{
  ...
  def write(out: Vector): Unit

  /** Write `out` to a 1D vector field at (`col`). */
  def write(col: Int, out: Vector): Unit

  /** Write `out` to a 2D vector field at (`row`, `col`). */
  def write(row: Int, col: Int, out: Vector): Unit

  /** Write `out` to a 3D vector field at (`row`, `col`). */
  def write(layer: Int, row: Int, col: Int, out: Vector): Unit
}