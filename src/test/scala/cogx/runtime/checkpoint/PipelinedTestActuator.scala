...
/** An pipelined actuator class set up as its own class, not an anonymous subclass of Actuator.
  *
  * A simpler approach that only requires a factory object is shown in SimpleTestActuator.
  *
  * @author Dick Carter
  */
class PipelinedTestActuator(source: Field, update: (Iterator[Float]) => Unit, resetHook: () => Unit,
                            parameters: () => String) extends Actuator(source, update, resetHook) {
  override def restoreParameters = parameters()

  // Note: this.getClass.getName will return "PipelinedTestActuator" as desired, so we need not override
...
/** Factory for a pipelined actuator class- creates instances from user code or from a stored parameter string.
  ...
  * @author Dick Carter
  */
object PipelinedTestActuator {
  /** The factory method for this actuator. */
  private def apply(source: Field, initState: Int): PipelinedTestActuator = {
    ...
    new PipelinedTestActuator(source, updateAndCheck _, reset _, parameters _)
  }

  /** The factory method used to create a pipelined actuator. */
  def apply(source: Field): PipelinedTestActuator = apply(source, 0)
