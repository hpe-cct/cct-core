...
/** An unpipelined actuator class set up as its own class, not an anonymous subclass of UnpipelinedActuator.
  *
  * A simpler approach that only requires a factory object is shown in SimpleTestActuator.
  *
  * @author Dick Carter
  */
class UnpipelinedTestActuator(source: Field, update: (Iterator[Float]) => Unit, resetHook: () => Unit,
                            parameters: () => String) extends UnpipelinedActuator(source, update, resetHook) {
  override def restoreParameters = parameters()

  // Note: this.getClass.getName will return "UnpipelinedTestActuator" as desired, so we need not override
...
/** Factory for an unpipelined actuator class- creates instances from user code or from a stored parameter string.
  ...
  * @author Dick Carter
  */
object UnpipelinedTestActuator {
  /** The factory method for this actuator. */
  private def apply(source: Field, initState: Int): UnpipelinedTestActuator = {
    ...
    new UnpipelinedTestActuator(source, updateAndCheck _, reset _, parameters _)
  }

  /** The factory method used to create an unpipelined actuator. */
  def apply(source: Field): UnpipelinedTestActuator = apply(source, 0)
