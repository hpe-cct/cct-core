...
import cogx.compiler.parser.syntaxtree.UnpipelinedSensor
import cogx.platform.types.FieldType

/** An unpipelined sensor class set up as its own class, not an anonymous subclass of UnpipelinedSensor.
  *
  * A simpler approach that only requires a factory object is shown in SimpleTestSensor.
  *
  * @author Dick Carter
  */
class UnpipelinedTestSensor(rows: Int, cols: Int,
                            nextValue: () => Iterator[Float],
                            reset: () => Unit,
                            parameters: () => String) extends UnpipelinedSensor(Shape(rows, cols), nextValue, reset) {
  override def restoreParameters = parameters()

  // Note: this.getClass.getName will return "UnpipelinedTestSensor" as desired, so we need not override
  // the restoringClass method.
}

/** Factory for an unpipelined sensor class- creates instances from user code or from a stored parameter string.
  *
  * @author Dick Carter
  */
object UnpipelinedTestSensor {
  ...
  def apply(rows: Int, cols: Int, initState: Int = 0) = {
    ...
    def nextValue() = new Iterator[Float] {
      var nextValue = state
      state += 1
      def next() = {
    ...
    new UnpipelinedTestSensor(rows, cols, nextValue, reset, parameters _)
  }
